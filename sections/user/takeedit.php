...
$Options['DisableLast5'] = (!empty($_POST['disablelast5']) ? 1 : 0);
$Options['DisableCoverArt'] = (!empty($_POST['disablecoverart']) ? 1 : 0);
...
// Country
if(array_key_exists($_POST['countrySelect'],$Countries)) {
        $Country=db_string($_POST['countrySelect']);
} else {
        $Country='';
}
// Timezone
if(isValidTimezone($_POST['timezone'])) {
	$Timezone=db_string($_POST['timezone']);
} else {
	$Timezone='';
}
