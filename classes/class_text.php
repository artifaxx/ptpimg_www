...
class TEXT {
	...
	private $ValidTags = array('b'=>0, 'u'=>0, 'i'=>0, 's'=>0, '*'=>0, '#'=>0, 'artist'=>0, 'user'=>0, 'n'=>0, 'inlineurl'=>0, 'inlinesize'=>1, 'align'=>1, 'color'=>1, 'colour'=>1, 'size'=>1, 'url'=>1, 'img'=>1, 'quote'=>1, 'pre'=>1, 'tex'=>0, 'hide'=>1, 'plain'=>0, 'important'=>0, 'br'=>0, 'pl'=>0, 'la'=>0
	);
	...
	function urls($Str) {
                if (strpos($Str,"javascript:")) {
                        $Str=substr($Str,0,strpos($Str,"javascript:")-1);
                }
...
*/
	...
	function to_html($Array) {
		...
		foreach($Array as $Block) {
			...
			switch($Block['Type']) {
				case 'la':
					$Str.='<strong><span style="color: #854250;">Gaga</span><span style="color: #3E7E74;">Edit:</span></strong><span style="color: #888243;"> '.$this->to_html($Block['Val']).'</span>';
					break;
				case 'pl':
					$Str.='<strong><span style="color: #996600;">pledgewise</span>edit:</strong><span style="color: #996600;"> '.$this->to_html($Block['Val']).'</span>';
					break;
				case 'br':
					$Str.='<strong><span style="color: #99FF66;">brownt0wn</span>edit:</strong><span style="color: #99FF66;"> '.$this->to_html($Block['Val']).'</span>';
					break;
				case 'b':
				...
				case 'quote':
					...
					if(!empty($Block['Attr'])) {
						$parse=$this->to_html($Block['Attr']);
						$checkInfo=strpos($parse,':');
						if($checkInfo===false){
							$Str.= '<strong>'.$this->to_html($Block['Attr']).'</strong> wrote: ';
						}
						else {
							$first=strpos($parse,':a');
							$second=strpos($parse,':b');
							$third=strpos($parse,':c');
							$postid=substr($parse,$first+2,$second-$first-2);		
							$threadid=substr($parse,$second+2,$third-$second-2);										
							$postnum=substr($parse,$third+2);
							$url='forums.php?action=viewthread&threadid='.$threadid.'&post='.$postnum.'#post'.$postid;	
							$attr=substr($parse,0,strpos($parse,':'));							
							$Str.= '<strong>'.$attr.'</strong> wrote: <a href="'.$url.'"><img border="0" src="'.STATIC_SERVER.'common/viewpost-right.png" alt="" /></a>';
						}
